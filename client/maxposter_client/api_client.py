...
class ApiClient:
    ...
    def list_pending_users(self):
        resp = requests.get(f"{self.base_url}/auth/pending", headers=self._headers(), timeout=self.timeout)
        self._handle_response(resp)
        return resp.json()

    def approve_user(self, user_id: int):
        resp = requests.post(
            f"{self.base_url}/auth/approve/{user_id}", headers=self._headers(), timeout=self.timeout
        )
        self._handle_response(resp)
        return resp.json()
    ...
    def reject_user(self, user_id: int):
        resp = requests.delete(
            f"{self.base_url}/auth/users/{user_id}", headers=self._headers(), timeout=self.timeout
        )
        self._handle_response(resp)
        return resp.json()

    def list_posters(self, category="all", search="", folder_id=None, page=1, page_size=20):
    ...
    def list_folders(self):
        resp = requests.get(f"{self.base_url}/folders", headers=self._headers(), timeout=self.timeout)
        self._handle_response(resp)
        return resp.json()

    def create_folder(self, name: str):
        resp = requests.post(
            f"{self.base_url}/folders", json={"name": name}, headers=self._headers(), timeout=self.timeout
        )
        self._handle_response(resp)
        return resp.json()
    ...
    def upload_poster(self, filepath: str, tags: str = "", folder_id=None):
