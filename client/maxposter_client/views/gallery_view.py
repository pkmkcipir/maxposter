...
class PosterCard(ctk.CTkFrame):
    def __init__(self, master, poster, on_click):
        ...
        display_folder = folder_name if folder_name else "\u2014"
        if folder_name and len(folder_name) > 24:
            display_folder = folder_name[:21] + "..."
        folder_label = ctk.CTkLabel(self, text=display_folder, font=ctk.CTkFont(size=10), text_color="gray")
        folder_label.pack()

        for widget in (self, self.image_label, name_label, badge, folder_label):
